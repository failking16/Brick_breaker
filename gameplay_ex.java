...
public class gameplay_ex extends JPanel implements KeyListener,ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		timer.start();
		if(play) {
			if(new Rectangle(ball_x,ball_y,20,20).intersects(new Rectangle(playerX,550,100,8))) {
			...
			for(int i =0;i<map.map.length;i++) {
				for(int j =0; j<map.map[0].length;j++) {
					if(map.map[i][j]>0) {
						...
						Rectangle ballrec= new Rectangle(ball_x,ball_y,20,320);
